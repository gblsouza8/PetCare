...
class VetClinicApp:
    ...
    def setup_aba_recepcao(self):
        ...
        # BOTÃO "Finalizar e Pagar" REMOVIDO: A finalização agora é feita pelo Veterinário.
    ...
    def carregar_consultas(self):
        ...
        try:
            ...
            # A ordenação principal é DECRESCENTE para Em Atendimento e Finalizadas (mais recentes no topo)
            ...
            # 1. Agendamentos (Ordem CRESCENTE, como solicitado - mais próximos primeiro)
            # A lista temporária está em ordem DECRESCENTE
    ...
    # def mover_para_finalizadas(self): <- FUNÇÃO REMOVIDA
    ...
    def carregar_consultas_para_relatorio(self):
        ...
        try:
            ...
            response = self.supabase.from_('consultas').select(
                'id, data_consulta, hora_consulta, status, tutores(nome), pets(nome), especialidades(nome)'
            ).in_('status', [2, 3]).order('data_consulta', desc=True).order('hora_consulta', desc=True).execute()
    ...
    def salvar_relatorio(self):
        ...
        try:
            ...
            if response.data:
                ...
                # 2. ALTERAÇÃO PRINCIPAL: Mudar o status da consulta para FINALIZADA (3)
                ...
                if update_status_response.data:
                    ...
                    self.carregar_consultas_para_relatorio() # Atualiza a lista do veterinário
                else:
