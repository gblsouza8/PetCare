...
class VetClinicApp:
    ...
    def setup_aba_recepcao(self):
        ...
        # BOTÃO DE ATUALIZAÇÃO ADICIONADO AQUI
        ...
        self.btn_finalizar_atendimento = ttk.Button(self.frame_atendimento, text="Finalizar e Pagar", command=self.mover_para_finalizadas)
        self.btn_finalizar_atendimento.pack(pady=5)
    ...
    def _format_consulta_display(self, item, include_tutor=True):
        """Formata o texto de exibição, tratando nulos nas relações."""
        
        # Extrai nomes de forma segura, usando .get() com fallback
        tutor_nome = item.get('tutores', {}).get('nome', '[Tutor N/A]') if item.get('tutores') else '[Tutor N/A]'
    ...
    def carregar_consultas(self):
        ...
        try:
            ...
            ).order('data_consulta', desc=False).order('hora_consulta', desc=False).execute()
            ...
            for item in data:
                try:
                    ...
                    if item['status'] == 1: # Agendada
                        self.list_agendamentos.insert(tk.END, display_text)
                    elif item['status'] == 2: # Em Atendimento
                        self.list_atendimento.insert(tk.END, display_text)
                    elif item['status'] == 3: # Finalizada
                        self.list_finalizadas.insert(tk.END, display_text)
    ...
    def mover_para_finalizadas(self):
        self._update_consulta_status(self.list_atendimento, 3)
    ...
    def setup_aba_veterinario(self):
        ...
        ttk.Label(self.relatorio_frame, text="1. Selecione a Consulta:").grid(row=1, column=0, sticky="nw", padx=5)
    ...
    def carregar_consultas_para_relatorio(self):
        ...
        try:
            # Buscar consultas que estão em atendimento (2) ou finalizadas (3)
            response = self.supabase.from_('consultas').select(
                'id, data_consulta, hora_consulta, status, tutores(nome), pets(nome), especialidades(nome)'
            ).in_('status', [2, 3]).order('data_consulta', desc=False).order('hora_consulta', desc=False).execute()
            ...
            if not data:
                print("DEBUG: Nenhuma consulta Em Atendimento/Finalizada encontrada no banco de dados.")
                self.vet_consulta_list.insert(tk.END, "Nenhuma consulta para relatório.")
                return

            for i, item in enumerate(data):
                try:
                    # Não incluímos o nome do tutor aqui, pois a lista do veterinário foca no Pet
    ...
    def salvar_relatorio(self):
        ...
        try:
            # Tentar inserir/atualizar (upsert) o relatório
            # Usando a tabela 'relatorios' e o campo único 'relatorio'
            ...
            if response.data:
                messagebox.showinfo("Sucesso", f"Relatório salvo com sucesso para a consulta ID: {consulta_id}.")
            else:
